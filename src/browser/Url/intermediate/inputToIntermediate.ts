...
function partsToIntermediate(parts: Url.Parts): $Url {
...
function optionsToIntermediate(options: Url.Options): $Url {
  const intermediate = jsUrlToIntermediate(new globalThis.URL(options.url));
...
function jsUrlToIntermediate(jsUrl: globalThis.URL): $Url {
  return partsToIntermediate({
    hash: jsUrl.hash,
    ...
    searchParams: jsUrl.searchParams,
    username: jsUrl.username,
  });
}

export function inputToIntermediate(input: Url.Input): $Url {
  if (input instanceof globalThis.URL) {
    return jsUrlToIntermediate(input);
  }
  ...
  if (typeof input === 'string') {
    return jsUrlToIntermediate(new globalThis.URL(input));
  }
