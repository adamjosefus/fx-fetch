...
describe('Url.format', () => {
  ...
  // Known divergence: IDNA/punycode is out of scope. `src/core` is multi-runtime and
  // must not use `node:url` domainToASCII; we have no in-house punycode
  // implementation yet. The hostname is only lowercased.
  test.skip('unicode hostname becomes punycode', () => {
    const input = 'https://háčkyčárky.cz/path';
  ...
  // Known divergence: SearchParams is a Map<string, string[]>, so a value-less
  // key `empty` is stored as `['empty', '']` and serialized back as `empty=`.
  // globalThis.URL preserves the raw query string, so it returns `empty` without `=`.
  test.skip('bare query key without "=" (SearchParams model limitation)', () => {
    const input = 'https://example.com/?empty';
