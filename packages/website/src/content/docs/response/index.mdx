...
:::tip
If you want to read JSON with a specific schema, consider using:
- [`readJsonWithSchema`](#readjsonwithschema) for Effect Schema
:::
...
//       │    >
//       ▼
const payload = Response.readJsonWithSchema(response, UserSchema);
```

### readJsonWithStandardSchemaV1

The `readJsonWithStandardSchemaV1` function reads the body of a `Response`, parses it as JSON,
and validates it against a provided [Standard Schema](https://standardschema.dev/) compatible schema.

This function works with any schema library that implements the Standard Schema specification,
including [Zod](https://zod.dev/), [Valibot](https://valibot.dev/), [ArkType](https://arktype.io/), and others.
...
### readStream
The `readStream` function reads the body of a `Response` and parses it as an Effect [Stream](https://effect.website/docs/stream/introduction/).
...
//       │    >
//       ▼
const payload = Response.readStream(response, {
