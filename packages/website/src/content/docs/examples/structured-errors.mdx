...
1. Define `withStructuredError` function.
   ...
   function withStructuredErrorFn<A, E, R, TError>(
     self: Effect.Effect<A, E | Response.NotOkError, R>, // ◀︎── Input must accept Response.NotOkError
     errorCode: string,
     onError: () => TError
   ): Effect.Effect<A, E | Response.NotOkError | TError, R> { // ◀︎── Same as input plus TError in the union
     // TODO: Implement
   }
   ```
...
4. Add dual API support
   ...
   Adding dual API support is super simple because EffectTS `Function` module.
...
```ts twoslash collapse={5-6} collapse={10-36} showLineNumbers=true
...
- `Response` can be safely read multiple times.
- `Response` cannot be mutated.
- `Response.readJsonWithSchema` gives you a simple way to parse and validate JSON payloads.
  - You cannot forget to handle all possible errors.
:::
