...
```ts twoslash showLineNumbers=false {3, 6-11} "signal"
import { Data, Effect } from 'effect';
...
This is safer, but still not usefull enough. What does `MyError` mean? Can I retry?
...
In Untype NPM, we have the [`@types/*` initiative](https://github.com/DefinitelyTyped/DefinitelyTyped).
